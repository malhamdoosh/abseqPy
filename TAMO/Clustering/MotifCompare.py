...
def diffrange(self,other,Srange,Orange):
    ...
    if (divisor <= 0):
    # Monther's Hack
#         print(self.id, self, other.id, other, 
#               [x.id + ' ' + str(x) for x in other.members])
        return 1
    #print(Srange)
    #print(Orange)    
    '''Computes distance'''
...
def minshortestoverhangdiff(A,B,minoverlap=6,want_offset=None,DFUNC=None):
    ...
    # Monther's Hack
    # if 1
    if 0:  #MEMOize rc when doing many, many comparisons (factor x2.5 speedup)
...
# Monther's Hack
def averagemotifs_counts(motifs,ovlp=2,template=None,DFUNC=negcommonbitsrange,VERBOSE=1,prop=''):
    if not template: 
    ...
    AVE = MotifTools.sum_counts(pmotifs,[])
    if VERBOSE:
        for m in pmotifs:
